...
const MysqlFields = require("./mysql.fields.js");
...
class Models {
    ...
    getSingletonTableFields(args) {
        ...
        // 查询字段
        return self.instance.getConfigFields(instanceConfig.tableName).then(fields => {
            if (fields.length == 0) {
                return Promise.reject(new Error("未查询到表字段数据"));
            }
            // 处理字段列表
            let fieldModel = `${args.language}Model${args.framework || ""}`;         // 如果一种语言超过
            // 检查model处理方法是否存在
            const modFields = MysqlFields[fieldModel];       // 
            // 判断方法是否存在
            if (!modFields) {
                return Promise.reject(new Error(`未找到${args.language}Model${args.framework || ""}，请检查开发语言以及框架模型是否支持代码自动创建`));
            }
            // 处理字段 
