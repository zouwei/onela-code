...
class MysqlFields {
  ...
  static nodeModel(tableName, fields) {
    ...
    for (let i of fields) {
      ...
      // 是否为主键
      if (i.COLUMN_KEY == "PRI") item.other += `, primaryKey: true`;
      // 是否可以为空
    ...
    // 读取模板内容
    return MysqlFields.getTemplate(templatePath)
  ...
  static denoModel(tableName, fields) {
    ...
    for (let i of fields) {
      ...
      if (i.COLUMN_KEY == "PRI") item.other += ', "primaryKey": true';
      // 是否主键自增
      if (i.EXTRA == "auto_increment") item.other += ', "autoIncrement": true';
      new_param.fields.push(item);
    }
  ...
  static csharpModel(tableName, fields) {
    ...
    for (let i of fields) {
      let item = {
        ...
        comment: i.COLUMN_COMMENT, // 字段备注
        other: " ",
      };

      // 是否为主键
      if (i.COLUMN_KEY == "PRI") new_param.primaryKey = i.COLUMN_NAME; // 主键名称
      // // 是否主键自增
      // if (i.EXTRA == "auto_increment")
    ...
    // 读取模板路径
    let templatePath = __dirname + "\\templates\\cshap.models.template";
    // 读取模板内容
    return MysqlFields.getTemplate(templatePath)
      .then((templateContent) => {
        new_param.templateContent = templateContent; // 模板内容
