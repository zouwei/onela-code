...
class MysqlFields {
    ...
    */
    static nodeModel(tableName, fields) {
        ...
        for (let i of fields) {
            // 局部变量
            let _datatype = DataType.getNodeDataTypeByMysql(i.DATA_TYPE, i.CHARACTER_MAXIMUM_LENGTH);
        ...
        // 读取模板路径
        let templatePath = __dirname + "\\templates\\node.models.sequelize.template";
        // 读取模板内容
        return MysqlFields.getTemplate(templatePath).then(templateContent => {
    ...
    */
    static denoModel(tableName, fields) {
        ...
        for (let i of fields) {
            let item = {
                "name": i.COLUMN_NAME,
                "type": DataType.getDatabaseTypeConvert(i.DATA_TYPE, "DENOTYPEBYMYSQL").toUpperCase(),
                "default": i.COLUMN_DEFAULT ? "\"" + i.COLUMN_DEFAULT + "\"" : "null",
                "comment": i.COLUMN_COMMENT,
                "other": " "
            };
        ...
        // 读取模板路径
        let templatePath = __dirname + "\\templates\\deno.models.template";
        // 读取模板内容
        return MysqlFields.getTemplate(templatePath).then(templateContent => {
            new_param.templateContent = templateContent;    // 模板内容
            // 返回页面元素替换参数
    ...
    static javaModel(tableName, fields) {
        ...
        // 私有属性、共有属性
        let new_param = {}, private_args = [], public_args = [];
        // 遍历
        for (let i of fields) {
            // 局部变量
            ...
            public_args.push(`	 */`);
            public_args.push(`	public void set${_humpname}(${_datatype} ${_columnname}) {`);
        ...
        // 读取模板路径
        let templatePath = __dirname + "\\templates\\java.models.template";
        // 读取模板内容
        return MysqlFields.getTemplate(templatePath).then(templateContent => {
            new_param.templateContent = templateContent;    // 模板内容
            // 返回页面元素替换参数
    ...
    static javaMapper(tableName, fields) {
        ...
        for (let i of fields) {
            ...
            if (i.COLUMN_KEY == "PRI") {
                // （map映射字段—）
                field_args.push(`		<id column="${i.COLUMN_NAME}" property="${_columnname}" jdbcType="${_datatype}" />     <!--${i.COLUMN_COMMENT}-->`);
            } else {
                field_args.push(`		<result column="${i.COLUMN_NAME}" property="${_columnname}" jdbcType="${_datatype}" />     <!--${i.COLUMN_COMMENT}-->`);
            }


            if (full_column_name != "valid") {
                insert_field
                insert_field.push(`			<if test="${_columnname} != null">${i.COLUMN_NAME},</if>`);
                values_content
                values_content.push(`			<if test="${_columnname} != null">#{${_columnname},jdbcType=${_datatype}},</if>`);
            }
            ...
            if (full_column_name != "id" && full_column_name != "valid") {
                // 查询记录列表
                where_query_record.push(`		<if test="${_columnname}!=null and ${_columnname}!=''">`);
                where_query_record.push(`		 	and ${i.COLUMN_NAME} =#{${_columnname},jdbcType=${_datatype}}`);
                where_query_record.push(`		</if>`);

                // 查询记录存在的列表
                where_query_exist.push(`		<choose>`);
                where_query_exist.push(`			<when test="${_columnname} ==null  or ${_columnname} ==''">`);
                where_query_exist.push(`				and ( ${i.COLUMN_NAME} is null  or ${i.COLUMN_NAME}='' )`);
                where_query_exist.push(`			</when>`);
                where_query_exist.push(`			<when test="${_columnname}!=null  and ${_columnname} !=''">`);
                where_query_exist.push(`				and ${i.COLUMN_NAME} = #{${i.COLUMN_NAME},jdbcType=${_datatype}}`);
                where_query_exist.push(`			</when>`);
                where_query_exist.push(`		</choose>`);
            }
        ...
        // 输出文件名
        new_param.fileNmme = DataName.NamingRules(tableName, 1) + ".xml";            // 可以自行添加后缀

        // 读取模板路径

        let templatePath = __dirname + "\\templates\\java.mapper.template";
        // 读取模板内容
        return MysqlFields.getTemplate(templatePath).then(templateContent => {
            new_param.templateContent = templateContent;    // 模板内容
            // 返回页面元素替换参数
